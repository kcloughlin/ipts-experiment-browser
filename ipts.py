...
class Presenter:
    ...
    def plot(self, gonio_values, gonio_names, run_numbers_list, scale_values):
        ...
        if len(self.model.subplot_limits) == 1:
                    ...
                    for ii in range(len(run_numbers_list)):
                        if ii == 0:
                            ax1.errorbar(
                                ...
                                v[0][ii],
                                yerr=np.array(
                                    [abs(v[1][ii] - v[0][ii]), abs(v[2][ii] - v[0][ii])]
                                ),
                                fmt=".",
                                color=c,
                                label=lab,
            ...
            ax1.set_xlim(
                self.model.subplot_limits[0][0] - 1, self.model.subplot_limits[0][1] + 1
            )
            ...
            color = "r"
            ax2.set_ylabel(
                f'Scale ({self.inst_params["Scale"].split(".")[-1]})', color=color
            )
            ...
            axs = self.view.plot.figure.subplots(
                ...
                sharey=True,
                width_ratios=[l[1] - l[0] + 2 for l in self.model.subplot_limits],
            )
            ...
            for i, ax in enumerate(axs):
                ...
                if i == 0:
                    ...
                    if i != len(self.model.subplot_limits) - 1:
                        ...
                        ax.plot(
                            [0, 0, 1, 1], [0, 1, 0, 1], transform=ax.transAxes, **kwargs
                        )
                    else:
                        ax.spines.right.set_visible(False)
                        ax.tick_params(axis="y", length=0)
    ...
    def sign_in(self):
        ...
        oncat = pyoncat.ONCat(
            ONCAT_URL, client_id=CLIENT_ID, flow=pyoncat.RESOURCE_OWNER_CREDENTIALS_FLOW
        )
...
class Model:
    ...
    def retrieve_data_files(self, login, inst_params, ipts_number):
        ...
        projection = [run_number, inst_params["Title"], inst_params["Scale"], "id"]
